...
<head>
    ...
    <style>
        ...
        *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .sidebar h1 { font-size: 32px; font-weight: 900; line-height: 1.2; margin-bottom: 10px; }
        .sidebar p.subtitle { font-size: 14px; font-weight: 600; margin-bottom: 20px; }

        .search-box {
            width: 100%; padding: 12px 15px; border: 1px solid var(--primary-orange); border-radius: 8px; font-size: 13px; margin-bottom: 30px; outline: none; background: var(--white); font-family: 'Nunito', sans-serif;
        }

        .filter-container { background-color: var(--box-yellow); border-radius: 12px; overflow: hidden; border: 1px solid #EAE3D9; margin-bottom: 30px; }
        .filter-header { background-color: var(--box-green-header); padding: 15px 20px; display: flex; justify-content: space-between; align-items: center; font-weight: 900; }
        .filter-header a { font-size: 12px; font-weight: 700; color: var(--text-dark); text-decoration: none; }
        .filter-header a:hover { color: var(--primary-orange); }
        .filter-body { padding: 20px; }
        .filter-group { margin-bottom: 20px; }
        .filter-group-title { display: flex; justify-content: space-between; font-weight: 800; font-size: 14px; margin-bottom: 10px; text-transform: uppercase; cursor: pointer; user-select: none; }
        .filter-items { display: flex; flex-direction: column; gap: 0; }
        .filter-item { display: flex; align-items: center; gap: 10px; margin-bottom: 8px; font-size: 13px; font-weight: 600; cursor: pointer; }
        .filter-item input[type="radio"] { width: 16px; height: 16px; accent-color: var(--primary-orange); cursor: pointer; }

        .btn-apply { width: 100%; background-color: #EAE6D1; border: 1px solid #EAE3D9; padding: 12px; border-radius: 8px; font-weight: 800; cursor: pointer; transition: .2s; font-size: 13px; font-family: 'Nunito', sans-serif; }
        .btn-apply:hover { background-color: #d5d0bb; }

        .info-box { background-color: #F8E1DF; border-radius: 12px; padding: 25px; border: 1px solid #E6B0AA; text-align: center; }
        ...
        .shop-banner { background-color: var(--box-yellow); border: 1px solid #EAE3D9; border-radius: 15px; padding: 40px; display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; position: relative; overflow: hidden; }
        .banner-text { position: relative; z-index: 2; max-width: 420px; }
        .banner-text h2 { font-size: 28px; font-weight: 900; margin-bottom: 10px; }
        .banner-text p { font-size: 14px; font-weight: 600; }
        .banner-image { position: absolute; right: -20px; top: 0; height: 110%; mix-blend-mode: multiply; z-index: 1; opacity: .85; }

        .results-bar { display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; font-size: 13px; font-weight: 600; }
        .sort-form { display: flex; align-items: center; gap: 8px; }
        .sort-dropdown { padding: 8px 12px; border-radius: 8px; border: 1px solid #EAE3D9; font-family: 'Nunito', sans-serif; font-weight: 700; font-size: 13px; outline: none; cursor: pointer; }

        .product-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 20px; }
        .product-card { background-color: var(--white); border-radius: 20px; overflow: hidden; border: 1px solid #EAE3D9; display: flex; flex-direction: column; transition: transform .2s, box-shadow .2s; }
        .product-card:hover { transform: translateY(-5px); box-shadow: 0 10px 20px rgba(0,0,0,.05); }

        .product-img-box { width: 100%; height: 200px; background-color: #F8F8F8; display: flex; justify-content: center; align-items: center; padding: 20px; flex-shrink: 0; }
        ...
        .tag-category { padding: 3px 10px; border-radius: 4px; font-size: 9px; font-weight: 800; text-transform: uppercase; background: #EAE6D1; color: var(--text-dark); }
        .tag-pet-type { padding: 3px 10px; border-radius: 4px; font-size: 9px; font-weight: 800; text-transform: uppercase; background: #FEE8BE; color: var(--text-dark); }

        .product-title { font-size: 15px; font-weight: 800; line-height: 1.25; margin-bottom: 5px; color: var(--text-dark); }
        .product-subtitle { font-size: 12px; font-weight: 600; color: var(--text-light); margin-bottom: 6px; }

        .product-rating { font-size: 12px; font-weight: 700; color: var(--primary-orange); margin-bottom: 10px; display: flex; align-items: center; gap: 4px; }
        .product-rating .review-count { color: var(--text-light); font-size: 11px; }

        .product-desc { font-size: 12px; font-weight: 600; color: var(--text-light); line-height: 1.45; margin-bottom: 15px; display: -webkit-box; -webkit-line-clamp: 2; line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; }

        .price-action-row { display: flex; justify-content: space-between; align-items: center; margin-top: auto; gap: 10px; }
        .product-price { font-size: 20px; font-weight: 900; color: var(--primary-orange); white-space: nowrap; }

        .card-btns { display: flex; gap: 8px; align-items: center; }
        .btn-view { background-color: var(--primary-orange); color: var(--white); padding: 8px 18px; border-radius: 20px; border: none; font-weight: 900; font-size: 12px; cursor: pointer; text-decoration: none; text-align: center; display: inline-flex; align-items: center; transition: .2s; font-family: 'Nunito', sans-serif; white-space: nowrap; }
        .btn-view:hover { background-color: var(--primary-hover); }

        .btn-add-cart { background-color: var(--btn-green); color: var(--white); width: 34px; height: 34px; border-radius: 50%; border: none; display: flex; justify-content: center; align-items: center; cursor: pointer; transition: .2s; font-size: 14px; flex-shrink: 0; }
        .btn-add-cart:hover { background-color: var(--btn-green-hover); transform: scale(1.1); }

        .no-results { text-align: center; padding: 60px 20px; color: var(--text-light); font-size: 15px; font-weight: 600; }
        ...
        .toast-accent { width: 5px; align-self: stretch; background: #4CAF7D; flex-shrink: 0; }
        .toast-icon { width: 42px; height: 42px; border-radius: 50%; background: #edfaf3; display: flex; align-items: center; justify-content: center; font-size: 17px; color: #4CAF7D; flex-shrink: 0; margin: 14px 4px 14px 14px; }
        .toast-text { flex: 1; padding: 14px 6px 14px 10px; }
        .toast-title { font-size: 13.5px; font-weight: 900; color: #2d2d2d; margin-bottom: 2px; }
        .toast-sub { font-size: 12px; font-weight: 600; color: #888; }
        .toast-close { background: none; border: none; cursor: pointer; font-size: 15px; color: #bbb; padding: 14px 14px 14px 6px; line-height: 1; transition: color .15s; align-self: flex-start; }
        ...
        @media (max-width: 768px) { .page-layout { flex-direction: column; } .sidebar { flex: none; width: 100%; position: static; } }
        @media (max-width: 480px) { .product-grid { grid-template-columns: 1fr; } #toastContainer { right: 10px; left: 10px; } .toast { min-width: unset; width: 100%; } }
    </style>
</head>
<body>
    <div class="container">
